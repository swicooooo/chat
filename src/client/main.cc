            ...
            else
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        //记录当前用户信息
                        g_current_user.id=response_js["id"].get<int>();
                        g_current_user.name=response_js["name"];
                        ...
                        if (response_js.contains("friends"))
                        {
                            ...
                            for (string &str : vec)
                            {
                                ...
                                User user;
                                user.id=friend_js["id"].get<int>();
                                user.name=friend_js["name"];
                                user.state=friend_js["state"];
                        ...
                        if (response_js.contains("groups"))
                        {
                            ...
                            for (string &str : vec)
                            {
                                ...
                                for (string &user_str : vec2)
                                {
                                    ...
                                    group_user.id = group_user_js["id"].get<int>();
                                    group_user.name=group_user_js["name"];
                                    group_user.state=group_user_js["state"];
                                    group_user.role=group_user_js["role"];

                                    group.groupUsers.push_back(group_user);
                                }
...
void ShowCurrentUserData()
{
    ...
    cout << "current login uer => id: " << g_current_user.id << " name: " << g_current_user.name << endl;
    cout << "-------------------friend  list-------------------" << endl;
    if (!g_current_friends_list.empty())
    {
        for (User &user : g_current_friends_list)
        {
            cout << user.id << " " << user.name << " " << user.state << endl;
        }
    }
    ...
    if (!g_current_group_list.empty())
    {
        ...
        {
            cout << group.id << " " << group.name << " " << group.desc << endl;

            //打印群员信息
            cout << "========group user========" << endl;
            for (GroupUser &group_user : group.groupUsers)
            {
                cout << group_user.id << " " << group_user.name << " " << group_user.state << " " << group_user.role << endl;
            }
        }
    }
    cout << "--------------------------------------------------" << endl;
